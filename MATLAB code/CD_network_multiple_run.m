...
for i = 1:1    
    ...
    for i = 1:test_size
        ...
        if (max_val < max(max(X_test{i})))
            max_val = max(max(X_test{i}));
        end
    end
    merr = err / test_size;
    ...
    max_val = intmin;
    for i = 1:test_size
        err = err + sum(sum(abs(X_reconstructed_reconstructed{i} - X_test{i})));
        if (max_val < max(max(X_test{i})))
            max_val = max(max(X_test{i}));
        end
    end
    ...
    max_val = intmin;
    for i = 1:10
        err = err + sum(sum(abs(X_reconstructed_reconstructed_reconstructed{i} - X_test{i})));
        if (max_val < max(max(X_test{i})))
            max_val = max(max(X_test{i}));
        end
    end
    ...
    max_val = intmin;
    for i = 1:test_size
        err = err + sum(abs(Y_pred{i} - Y_test(i)));
        if (max_val < max(Y_test(i)))
    ...
    a1 = autoencoder_layer_1_relative_mean_error;
    a2 = autoencoder_layer_2_relative_mean_error;
    a3 = autoencoder_layer_3_relative_mean_error;
